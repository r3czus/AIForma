...
public sealed record ProductResponse(Guid Id, string Name, string? Brand, decimal CaloriesPer100, decimal ProteinPer100, decimal FatPer100, decimal CarbohydratesPer100, decimal? DefaultServingAmount = null, ServingUnit DefaultServingUnit = ServingUnit.Gram, decimal? GramsPerPiece = null, string? Barcode = null, bool IsOwn = false);
...
public sealed record NutritionDayResponse(DateOnly Date, MacroResponse? Target, MacroResponse Consumed, MacroResponse? Remaining, IReadOnlyList<MealResponse> Meals, NutritionDayStatus Status = NutritionDayStatus.Unknown);
public sealed record SaveNutritionDayStatusRequest(NutritionDayStatus Status);
